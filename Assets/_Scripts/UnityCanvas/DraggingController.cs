...
namespace UnityExpansion
{
    ...
    public class DraggingController : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
    {
        // 被拖曳的物件
        public RectTransform item;

        // 被拖曳物件偏離欄位中心多少距離，啟用交換機制
        public int swap_distance = 10;
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            for (int i = 0; i < n_child; i++)
            {
                ...
                if (i != item.GetSiblingIndex())
                {
                    ...
                    int distance = (int)Vector3.Distance(item.position, other.position);
